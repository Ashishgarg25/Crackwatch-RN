...
export default class UnCracked extends React.Component{
    ...
    componentDidMount(){
        fetch("https://api.crackwatch.com/api/games?page=0&is_cracked=false&sort_by=release_date&is_released=true")
    ...
    render(){
        ...
        return(
            <View style={styles.container}>
                <Text style={styles.heading}>UNCRACKED GAMES</Text>
                ...
                <ScrollView contentContainerStyle={{ flexDirection:"row", justifyContent:"space-evenly", alignItems:"center", flexWrap:"wrap"}}>
                    {this.state.uncracked.map(item => (
                        <TouchableOpacity style={{marginHorizontal:6, marginTop:32}} key={item.id} onPress={ () => this.gameDetails(item) }>
                            ...
                            <Text style={styles.title}>UNCRACKED D+{item.diffInDays}</Text>
                        </TouchableOpacity>
                    ))}
                </ScrollView>
