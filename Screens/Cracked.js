...
export default class Cracked extends React.Component{
    ...
    componentDidMount(){
        fetch("https://api.crackwatch.com/api/games?page=0&is_cracked=true&sort_by=release_date")
    ...
    render(){
        ...
        return(
            <View style={styles.container}>
                <Text style={styles.heading}>CRACKED GAMES</Text>
                ...
                <ScrollView contentContainerStyle={{ flexDirection:"row", justifyContent:"space-evenly", alignItems:"center", flexWrap:"wrap"}}>
                    {this.state.cracked.map(item => (
                        <TouchableOpacity style={{marginHorizontal:6, marginTop:32}} key={item.id} onPress={ () => this.gameDetails(item) }>
                            ...
                            <Text style={styles.title}>CRACKED D+{item.diffInDays}</Text>
                        </TouchableOpacity>
                    ))}
                </ScrollView>
